...
//------------------------------------------------------------------------------
vtkMRMLLITTPlanManagerNode* vtkMRMLLITTPlanManagerNode::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkMRMLLITTPlanManagerNode"); if(ret)
    {
      return (vtkMRMLLITTPlanManagerNode*)ret;
    }
  // If the factory was unable to create the object, then create it here.
  return new vtkMRMLLITTPlanManagerNode;
}

//----------------------------------------------------------------------------
vtkMRMLNode* vtkMRMLLITTPlanManagerNode::CreateNodeInstance()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkMRMLLITTPlanManagerNode");
  if(ret)
    {
      return (vtkMRMLLITTPlanManagerNode*)ret;
    }
...
//----------------------------------------------------------------------------
vtkMRMLLITTPlanManagerNode::vtkMRMLLITTPlanManagerNode()
{
...
{  
  ...
  for (std::vector<vtkLITTPlanTargetDescriptor*>::iterator it=this->TargetDescriptorsVector.begin(); it!=this->TargetDescriptorsVector.end(); ++it)
    {   
    if ((*it)!=NULL)
      {
      (*it)->Delete();
...
//----------------------------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::WriteXML(ostream& of, int nIndent)
{
  ...
  of << indent << " LITTPlanModuleVersion=\"" << GetLITTPlanModuleVersion() << "\"";
...
//----------------------------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::ReadXMLAttributes(const char** atts)
{
  ...
  while (*atts != NULL)
  {
    ...
    // LITTPlanModuleVersion - ignore (may be used in the future for backward compatibility)
    ...
    if (GetAttNameSection(attName,"Target", sectionInd, sectionName))
    {
      ...
      vtkLITTPlanTargetDescriptor *targetDesc=this->TargetDescriptorsVector[sectionInd];
      if (targetDesc==NULL)
      {
        // the descriptor has not been created yet
        targetDesc=vtkLITTPlanTargetDescriptor::New();
      ...
      else if (!sectionName.compare("FiducialID"))
      {
        ...
        // the FiducialID attribute and let vtkLITTPlanLogic::UpdateTargetListFromMRML
        // do the linking.
...
bool vtkMRMLLITTPlanManagerNode::GetAttNameSection(const std::string& attName, const std::string& groupName, unsigned int &sectionInd, std::string &sectionName)
{
...
void vtkMRMLLITTPlanManagerNode::Copy(vtkMRMLNode *anode)
{
  Superclass::Copy(anode);
  vtkMRMLLITTPlanManagerNode *node = (vtkMRMLLITTPlanManagerNode *) anode;
  ...
  this->TargetDescriptorsVector.clear();
  //copy the contents of the other
  vtkLITTPlanTargetDescriptor* tdesc=NULL;
  for (std::vector<vtkLITTPlanTargetDescriptor*>::iterator it=node->TargetDescriptorsVector.begin(); it!=node->TargetDescriptorsVector.end(); ++it)
    {    
    tdesc=vtkLITTPlanTargetDescriptor::New();
    tdesc->DeepCopy(*(*it));
...
//-----------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::UpdateReferences()
{
...
//----------------------------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::UpdateReferenceID(const char *oldID, const char *newID)
{
...
//-----------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::UpdateScene(vtkMRMLScene *scene)
{
...
//----------------------------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::ProcessMRMLEvents( vtkObject *caller, unsigned long event, void *callData )
{
...
//----------------------------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
int vtkMRMLLITTPlanManagerNode::GetNumberOfSteps()
{
...
//----------------------------------------------------------------------------
const char* vtkMRMLLITTPlanManagerNode::GetStepName(int i)
{
...
//----------------------------------------------------------------------------
int vtkMRMLLITTPlanManagerNode::SwitchStep(int newStep)
{
...
//----------------------------------------------------------------------------
int vtkMRMLLITTPlanManagerNode::GetCurrentStep()
{
  return this->CurrentStep;
}

//----------------------------------------------------------------------------
vtkStdString vtkMRMLLITTPlanManagerNode::GetWorkflowStepsString()
{
...
//----------------------------------------------------------------------------
bool vtkMRMLLITTPlanManagerNode::SetWorkflowStepsFromString(const vtkStdString& workflowStepsString)
{ 
  if (this->StepList==NULL)
  {
...
//----------------------------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::SetAndObserveTargetPlanListNodeID(const char *nodeID)
{
...
//----------------------------------------------------------------------------
vtkMRMLFiducialListNode* vtkMRMLLITTPlanManagerNode::GetTargetPlanListNode()
{
...
//----------------------------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::SetAndObserveRobotNodeID(const char *nodeID)
{
...
//----------------------------------------------------------------------------
vtkMRMLRobotNode* vtkMRMLLITTPlanManagerNode::GetRobotNode()
{
...
//-------------------------------------------------------------------------------
unsigned int vtkMRMLLITTPlanManagerNode::AddTargetDescriptor(vtkLITTPlanTargetDescriptor* target)
{
...
//-------------------------------------------------------------------------------
vtkLITTPlanTargetDescriptor* vtkMRMLLITTPlanManagerNode::GetTargetDescriptorAtIndex(unsigned int index)
{
...
//-------------------------------------------------------------------------------
int vtkMRMLLITTPlanManagerNode::RemoveTargetDescriptorAtIndex(unsigned int index)
{
  if (index >= this->TargetDescriptorsVector.size())
    {
    return 0;
    }
  vtkLITTPlanTargetDescriptor* targetDesc=GetTargetDescriptorAtIndex(index);
  if (targetDesc!=NULL)
  {
    targetDesc->Delete();
  }
...
//-------------------------------------------------------------------------------
int vtkMRMLLITTPlanManagerNode::SetCurrentTargetIndex(int index)
{
  ...
  if (GetRobotNode())
  {
    vtkLITTPlanTargetDescriptor* targetDesc=GetTargetDescriptorAtIndex(this->CurrentTargetIndex);
    NeedleDescriptorStruct *needle = GetNeedle(targetDesc); 
    GetRobotNode()->ShowRobotAtTarget(targetDesc, needle);
  }

  this->Modified();
  this->InvokeEvent(vtkMRMLLITTPlanManagerNode::CurrentTargetChangedEvent);
  return this->CurrentTargetIndex;
}

//-------------------------------------------------------------------------------
bool vtkMRMLLITTPlanManagerNode::AddTargetToFiducialList(double targetRAS[3], unsigned int fiducialListIndex, unsigned int targetNr, int & fiducialIndex)
{
...
//-------------------------------------------------------------------------------
bool vtkMRMLLITTPlanManagerNode::GetTargetFromFiducialList(int fiducialListIndex, int fiducialIndex, double &r, double &a, double &s)
{
...
//-------------------------------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::SetFiducialColor(int fiducialIndex, bool selected)
{
  vtkMRMLFiducialListNode* fidNode=this->GetTargetPlanListNode();
  if (fidNode==NULL)
  {
...
//------------------------------------------------------------------------------
void vtkMRMLLITTPlanManagerNode::Init(const char* defaultNeedleListConfig)
{
  if (this->Initialized)
  {
    return;
  ...
  if (GetTargetPlanListNode()==NULL && this->Scene!=NULL)
  {
    ...
    targetFidList->SetDescription("LITTPlan target point list");
...
bool vtkMRMLLITTPlanManagerNode::ReadNeedleListFromConfigXml(const char* needleListConfigStr)
{  
...
bool vtkMRMLLITTPlanManagerNode::SetNeedle(unsigned int needleIndex, NeedleDescriptorStruct needleDesc)
{
...
bool vtkMRMLLITTPlanManagerNode::GetNeedle(unsigned int needleIndex, NeedleDescriptorStruct &needleDesc)
{
  if (needleIndex >= this->NeedlesVector.size())
    {
    return false;
...
NeedleDescriptorStruct* vtkMRMLLITTPlanManagerNode::GetNeedle(vtkLITTPlanTargetDescriptor *targetDesc)
{
  if (targetDesc==NULL)
  {
    return NULL;
...
const char* vtkMRMLLITTPlanManagerNode::GetLITTPlanModuleVersion()
{
