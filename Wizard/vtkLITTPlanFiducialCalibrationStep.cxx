...
//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkLITTPlanFiducialCalibrationStep);
vtkCxxRevisionMacro(vtkLITTPlanFiducialCalibrationStep, "$Revision: 1.1 $");

//----------------------------------------------------------------------------
vtkLITTPlanFiducialCalibrationStep::vtkLITTPlanFiducialCalibrationStep()
{
  this->SetTitle("Applicator Placement");
  this->SetDescription("Load planning data and click on the markers to place the applicator.");  
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os,indent);
}

//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::ShowLoadResetControls()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::ShowLoadVolumeControls()
{
  vtkKWWidget *parent = this->GetGUI()->GetWizardWidget()->GetClientArea();
  ...
  if (!this->LoadCalibrationVolumeButton->IsCreated())
    ...
    this->LoadCalibrationVolumeButton->SetBalloonHelpString("Click to load a volume. Need to additionally select the volume to make it the current planning data.");
    }
  ...
  if (!this->ShowRobotButton->IsCreated()) {
    ...
    this->ShowRobotButton->SetBalloonHelpString("Display Applicator");
  }
  ...
  if (!this->ShowWorkspaceButton->IsCreated()) 
    ...
    this->ShowWorkspaceButton->SetText("Show Damage Region");
    this->ShowWorkspaceButton->SetBalloonHelpString("Display Model of Damage Region");
    }  
  ...
  if (!this->VolumeSelectorWidget->IsCreated())
    ...
    this->VolumeSelectorWidget->SetLabelText( "Planning Data: ");
    this->VolumeSelectorWidget->SetBalloonHelpString("Select the planning volume from the current scene.");    
    }
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::ShowFiducialSegmentParamsControls()
{
  vtkKWWidget *parent = this->GetGUI()->GetWizardWidget()->GetClientArea();
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::ShowFiducialSegmentationResultsControls()
{
  vtkKWWidget *parent = this->GetGUI()->GetWizardWidget()->GetClientArea();
  ...
  if (!this->SegmentationResultsFrame->IsCreated())
    ...
    this->SegmentationResultsFrame->SetLabelText("Planning Placement/Registration results");
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::ShowExportImportControls()
{
   vtkKWWidget *parent = this->GetGUI()->GetWizardWidget()->GetClientArea();
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::ShowUserInterface()
{
  ...
  vtkMRMLLaserApplicatorNode* robot=GetRobot();
  if(robot!=NULL)
  {    
...
//-------------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::HideUserInterface()
{
...
//----------------------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::TearDownGUI()
{
  RemoveMRMLObservers();
  RemoveGUIObservers();   
}
...
//----------------------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::PopulateCalibrationResults()
{
  std::ostrstream os;  
  
  vtkMRMLLaserApplicatorNode* robot=GetRobot();
  TRLaserAblationCalibrationData calibData;
  if (robot!=NULL)
  {    
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::AddGUIObservers()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::RemoveGUIObservers()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::Enter()
{  

}

//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::Exit()
{

}

//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::WizardGUICallback( vtkObject *caller,
                         unsigned long eid, void *clientData, void *callData )
{

  vtkLITTPlanFiducialCalibrationStep *self = reinterpret_cast<vtkLITTPlanFiducialCalibrationStep *>(clientData);
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::UpdateMRML() 
{
  vtkMRMLLaserApplicatorNode* robot=GetRobot();
  if(robot==NULL)
  {
  ...
  vtkMRMLLITTPlanManagerNode *managerNode = this->GetGUI()->GetLITTPlanManagerNode();
  if(managerNode==NULL)
  {
    return;
  }
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::UpdateGUI() 
{
  vtkMRMLLaserApplicatorNode* robot= GetRobot();
  if(robot==NULL)
  {
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::ProcessGUIEvents(vtkObject *caller,
        unsigned long event, void *callData)
{
  //slicerCerr("vtkLITTPlanFiducialCalibrationStep::ProcessGUIEvents");
...
//-----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::Reset()
{
  vtkMRMLLaserApplicatorNode* robot=GetRobot();
  if (robot==NULL)
  {
...
//-----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::Resegment()
{
...
//--------------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::ShowAxesIn3DView(bool show)
{
  vtkMRMLLaserApplicatorNode* robot = GetRobot();    
  if (robot)
  {
...
vtkMRMLLaserApplicatorNode* vtkLITTPlanFiducialCalibrationStep::GetRobot()
{
  if (this->GetGUI()==NULL)
  {
    return NULL;
  }
  if (this->GetGUI()->GetLITTPlanManagerNode()==NULL)
  {
    return NULL;
  }
  return vtkMRMLLaserApplicatorNode::SafeDownCast(this->GetGUI()->GetLITTPlanManagerNode()->GetRobotNode());
}

//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::ProcessMRMLEvents(vtkObject *caller,
                                         unsigned long event, void *callData)
{
  ...
  // Robot node changed
  vtkMRMLLaserApplicatorNode *robot = vtkMRMLLaserApplicatorNode::SafeDownCast(caller);
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::AddMRMLObservers()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::RemoveMRMLObservers()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanFiducialCalibrationStep::EnableMarkerPositionEdit(bool enable)
{  
...
void vtkLITTPlanFiducialCalibrationStep::JumpToFiducial(unsigned int fid1Index)
{
...
vtkMRMLFiducialListNode* vtkLITTPlanFiducialCalibrationStep::GetCalibrationPointListNode()
{
  vtkMRMLLaserApplicatorNode* robot=GetRobot();
  if(robot==NULL)
  {  
