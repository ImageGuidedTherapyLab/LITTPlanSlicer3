...
//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkLITTPlanStepVerification);
vtkCxxRevisionMacro(vtkLITTPlanStepVerification, "$Revision: 1.1 $");

//----------------------------------------------------------------------------
vtkLITTPlanStepVerification::vtkLITTPlanStepVerification()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::SetVerificationPointListNode(vtkMRMLFiducialListNode *node)
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::ShowUserInterface()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::ShowVolumeSelectionFrame()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::ShowTargetListFrame()
{
  vtkKWWidget *parent = this->GetGUI()->GetWizardWidget()->GetClientArea();
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::ShowVerificationControlFrame()
{
  vtkKWWidget *parent = this->GetGUI()->GetWizardWidget()->GetClientArea();
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::ProcessGUIEvents(vtkObject *caller,
                                          unsigned long event, void *callData)
{
  ...
  {    
    ...
    vtkLITTPlanTargetDescriptor* targetDesc=NULL;
    if (this->TargetIndexUnderVerification>=0)
    {
      targetDesc=this->GetLITTPlanManager()->GetTargetDescriptorAtIndex(this->TargetIndexUnderVerification);      
    }
    else
    {
      targetDesc=this->GetLITTPlanManager()->GetTargetDescriptorAtIndex(this->GetLITTPlanManager()->GetCurrentTargetIndex());
    }
    if (targetDesc!=NULL)
    {
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::ProcessMRMLEvents(vtkObject *caller,
                                         unsigned long event, void *callData)
{  
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::AddMRMLObservers()
{
  if (this->VerificationPointListNode!=NULL)
  {
    vtkSmartPointer<vtkIntArray> events = vtkSmartPointer<vtkIntArray>::New();
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::RemoveMRMLObservers()
{
  vtkMRMLLITTPlanManagerNode* manager=this->GetLITTPlanManager();
  if (manager!=NULL)
  {
...
//---------------------------------------------------------------------------
void vtkLITTPlanStepVerification::OnMultiColumnListSelection()
{
  ...
  if (numRows == 1)
    ...
    this->GUI->BringTargetToViewIn2DViews(vtkLITTPlanGUI::BRING_MARKERS_TO_VIEW_ALIGN_TO_NEEDLE);
    }
}

//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::UpdateTargetListGUI()
{
    ...
    if (target->GetTargetValidated())
      {
        ...
        {
          std::ostrstream os;    
          os << std::setiosflags(ios::fixed | ios::showpoint) << std::setprecision(vtkLITTPlanGUI::POSITION_PRECISION_DIGITS*2); // *2 to have double precision as the position that it was computed from
        ...
        {
        std::ostrstream os;    
        os << std::setiosflags(ios::fixed | ios::showpoint) << std::setprecision(vtkLITTPlanGUI::POSITION_PRECISION_DIGITS*3);
...
//-----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::AddGUIObservers()
{
...
//-----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::RemoveGUIObservers()
{
  if (this->LoadVerificationVolumeButton)
    {
...
//--------------------------------------------------------------------------------
void vtkLITTPlanStepVerification::UpdateGUI()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::HideUserInterface()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::TearDownGUI()
{  
  RemoveGUIObservers();
  RemoveMRMLObservers();
...
//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::StartVerification()
{
  ...
  if (this->GetLITTPlanManager()->GetCurrentTargetIndex()<0)
  {
    // no selected target
    return;
  }        
  ...
  this->TargetIndexUnderVerification=this->GetLITTPlanManager()->GetCurrentTargetIndex();
}

//----------------------------------------------------------------------------
void vtkLITTPlanStepVerification::StopVerification()
{
...
void vtkLITTPlanStepVerification::UpdateVerificationResultsForCurrentTarget()
{
  if (this->TargetIndexUnderVerification<0)
  {
  ...
  vtkLITTPlanTargetDescriptor* targetDesc=this->GetLITTPlanManager()->GetTargetDescriptorAtIndex(this->TargetIndexUnderVerification);
  ...
  overallErr = LITTPlanMath::ComputeDistanceLinePoint(targetDesc->GetNeedleTipValidationPosition(), targetDesc->GetNeedleBaseValidationPosition(),
  ...
  this->GetLITTPlanManager()->Modified();
}

void vtkLITTPlanStepVerification::DisplayVerificationResultsForCurrentTarget()
{
  if (this->TargetIndexUnderVerification>=0)
  {
  ...
  if (GetLITTPlanManager()==NULL)
  {
    vtkErrorMacro("Invalid manager");
    return;
  }

  int targetIndex=GetLITTPlanManager()->GetCurrentTargetIndex(); 
  vtkLITTPlanTargetDescriptor* targetDesc=NULL;
  if (targetIndex>=0)
  {
    targetDesc=this->GetLITTPlanManager()->GetTargetDescriptorAtIndex(targetIndex);    
  }
