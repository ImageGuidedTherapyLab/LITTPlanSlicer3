...
//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkLITTPlanTargetingStep);
vtkCxxRevisionMacro(vtkLITTPlanTargetingStep, "$Revision: 1.1 $");

//----------------------------------------------------------------------------
vtkLITTPlanTargetingStep::vtkLITTPlanTargetingStep()
{
  //this->SetName("Targeting");
  this->SetTitle("Bioheat Model");
  this->SetDescription("Set bioheat parameters.");
...
vtkLITTPlanTargetingStep::~vtkLITTPlanTargetingStep()
{
  ...
  DELETE_IF_NULL_WITH_SETPARENT_NULL(PerfusionValueBox);
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::ShowUserInterface()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::ShowTargetPlanningFrame()
{
  ...
  if (!this->ShowRobotButton->IsCreated()) {
    ...
    this->ShowRobotButton->SetText("Show Applicator");
    this->ShowRobotButton->SetBalloonHelpString("Display Laser Applicator");
  }
  ...
  if (!this->PerfusionValueBox)
    {
    this->PerfusionValueBox = vtkKWSpinBoxWithLabel::New();
    }
  if (!this->PerfusionValueBox->IsCreated())
    {
    this->PerfusionValueBox->SetParent(this->TargetPlanningFrame);
    this->PerfusionValueBox->Create();
  ...
  if (!this->ThermalConductivityValueBox)
    {
    this->ThermalConductivityValueBox = vtkKWSpinBoxWithLabel::New();
    }
  if (!this->ThermalConductivityValueBox->IsCreated())
    {
    this->ThermalConductivityValueBox->SetParent(this->TargetPlanningFrame);
    this->ThermalConductivityValueBox->Create();
    this->ThermalConductivityValueBox->GetWidget()->SetWidth(11);
  ...
  if (!this->OpticalAbsorptionValueBox)
    {
    this->OpticalAbsorptionValueBox = vtkKWSpinBoxWithLabel::New();
    }
  if (!this->OpticalAbsorptionValueBox->IsCreated())
    {
    this->OpticalAbsorptionValueBox->SetParent(this->TargetPlanningFrame);
    this->OpticalAbsorptionValueBox->Create();
    this->OpticalAbsorptionValueBox->GetWidget()->SetWidth(11);
    this->OpticalAbsorptionValueBox->GetWidget()->SetRange(0,1000);
    this->OpticalAbsorptionValueBox->GetWidget()->SetIncrement(0.5);
    this->OpticalAbsorptionValueBox->GetWidget()->SetValue(100);
    this->OpticalAbsorptionValueBox->SetLabelText("Optical Absorption [1/m]");
    }

  if (!this->OpticalScatteringValueBox)
    {
    this->OpticalScatteringValueBox = vtkKWSpinBoxWithLabel::New();
    }
  if (!this->OpticalScatteringValueBox->IsCreated())
    {
    this->OpticalScatteringValueBox->SetParent(this->TargetPlanningFrame);
    this->OpticalScatteringValueBox->Create();
    this->OpticalScatteringValueBox->GetWidget()->SetWidth(11);
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::ShowTargetListFrame()
{
  vtkKWWidget *parent = this->GetGUI()->GetWizardWidget()->GetClientArea();
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::ShowTargetControlFrame()
{
  vtkKWWidget *parent = this->GetGUI()->GetWizardWidget()->GetClientArea();
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::ProcessGUIEvents(vtkObject *caller,
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::ProcessMRMLEvents(vtkObject *caller,
                                         unsigned long event, void *callData)
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::AddMRMLObservers()
{
  ...
  if (manager->HasObserver(vtkMRMLLITTPlanManagerNode::CurrentTargetChangedEvent, (vtkCommand *)this->MRMLCallbackCommand) < 1)
  {
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::RemoveMRMLObservers()
{
...
//---------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::OnMultiColumnListUpdate(int row, int col, char * str)
{
  ...
  if ((row >= 0) && (row < this->TargetList->GetWidget()->GetNumberOfRows()) &&
    ...
    if (col == COL_NAME)
      ...
      {
        std::ostrstream os;    
        os << std::setiosflags(ios::fixed | ios::showpoint) << std::setprecision(vtkLITTPlanGUI::POSITION_PRECISION_DIGITS);
  ...
  if (updated)
  {
    ...
    int currentTarget=this->GetLITTPlanManager()->GetCurrentTargetIndex();
    this->GetLITTPlanManager()->SetCurrentTargetIndex(-1);
    this->GetLITTPlanManager()->SetCurrentTargetIndex(currentTarget);
  }
}

//---------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::OnMultiColumnListSelection()
{
  vtkMRMLFiducialListNode* fidList = this->GetLITTPlanManager()->GetTargetPlanListNode();
  ...
  if (numRows == 1)
    ...
    const char* volNodeID = this->GetLITTPlanManager()->GetTargetingVolumeNodeRef();
    ...
    */
    ...
    this->Script("after 1000 \"%s BringTargetToViewIn2DViews %i\"", this->GUI->GetTclName(), vtkLITTPlanGUI::BRING_MARKERS_TO_VIEW_KEEP_CURRENT_ORIENTATION);
    
    }
}

//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::UpdateTargetListGUI()
{
  ...
  {      
    ...
    if (xyz != NULL)
    {
      ...
      {
        if (deleteFlag || fabs(columnList->GetCellTextAsDouble(row,COL_X+i)-xyz[i])>vtkLITTPlanGUI::POSITION_PRECISION_TOLERANCE)
        {
          std::ostrstream os;    
          os << std::setiosflags(ios::fixed | ios::showpoint) << std::setprecision(vtkLITTPlanGUI::POSITION_PRECISION_DIGITS);
          os << xyz[i] << std::ends;
    ...
    if (this->ShowTargetOrientation && wxyz != NULL)
    {
      ...
      {
        if (deleteFlag || fabs(columnList->GetCellTextAsDouble(row, COL_OR_W+i)-wxyz[i])>vtkLITTPlanGUI::POSITION_PRECISION_TOLERANCE)
        {
          std::ostrstream os;    
          os << std::setiosflags(ios::fixed | ios::showpoint) << std::setprecision(vtkLITTPlanGUI::POSITION_PRECISION_DIGITS);
          os << wxyz[i] << std::ends;
  ...
  int currentTargetIndex=this->GetLITTPlanManager()->GetCurrentTargetIndex();
...
//-----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::AddGUIObservers()
{
...
//-----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::RemoveGUIObservers()
{
  if (this->LoadTargetingVolumeButton)
    {
...
//--------------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::UpdateGUI()
{
  vtkMRMLLITTPlanManagerNode *mrmlNode = this->GetGUI()->GetLITTPlanManagerNode();
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::HideUserInterface()
{
...
//----------------------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::TearDownGUI()
{
  RemoveMRMLObservers();
  RemoveGUIObservers();
}
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::EnableAddTargetsOnClickButton(bool enable)
{
  if (this->GetLITTPlanManager()==NULL)
  {
    return;
...
//----------------------------------------------------------------------------
void vtkLITTPlanTargetingStep::SetShowTargetOrientation(int show)
{
  if (this->TargetList)
  {
