...
#include "vtkLITTPlanStepSetUp.h"
#include "vtkLITTPlanStepSetUpTemplate.h"
#include "vtkLITTPlanStepVerification.h"
#include "vtkLITTPlanCalibrationStep.h"
#include "vtkLITTPlanFiducialCalibrationStep.h"
#include "vtkLITTPlanTargetingStep.h"
#include "vtkLITTPlanStepTargetingTemplate.h"
#include "vtkLITTPlanManualControlStep.h"
...
const int vtkLITTPlanGUI::POSITION_PRECISION_DIGITS=1;
const double vtkLITTPlanGUI::POSITION_PRECISION_TOLERANCE=0.1/2.0;
...
//---------------------------------------------------------------------------
vtkStandardNewMacro (vtkLITTPlanGUI );
vtkCxxRevisionMacro ( vtkLITTPlanGUI, "$Revision: 1.0 $");
//---------------------------------------------------------------------------


//---------------------------------------------------------------------------
vtkLITTPlanGUI::vtkLITTPlanGUI ( )
{
  ...
  this->DataCallbackCommand->SetCallback(vtkLITTPlanGUI::DataCallback);
  
  this->LITTPlanManagerNodeID =  NULL;
  this->LITTPlanManagerNode =  NULL;
  ...
  this->LITTPlanManagerSelectorWidget = NULL;
...
vtkLITTPlanGUI::~vtkLITTPlanGUI ( )
{
  ...
  if (this->LITTPlanManagerSelectorWidget)
    {
    this->LITTPlanManagerSelectorWidget->SetParent(NULL );
    this->LITTPlanManagerSelectorWidget->Delete ( );
    this->LITTPlanManagerSelectorWidget=NULL;
    }
  ...
  this->SetAndObserveLITTPlanManagerNodeID( NULL );
  this->SetAndObserveTargetPlanListNodeID( NULL );
}


//---------------------------------------------------------------------------
void vtkLITTPlanGUI::PrintSelf ( ostream& os, vtkIndent indent )
{
    this->vtkObject::PrintSelf ( os, indent );
    
    os << indent << "LITTPlanGUI: " << this->GetClassName ( ) << "\n";
    os << indent << "LITTPlanManager: ";
    if (this->LITTPlanManagerNodeID)
    {
      os << this->LITTPlanManagerNodeID << "\n";
    }
    else
    {
       os << "NULL\n";
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::RemoveGUIObservers ( )
{
  ...
  if (this->LITTPlanManagerSelectorWidget)
    {
    this->LITTPlanManagerSelectorWidget->RemoveObservers ( vtkSlicerNodeSelectorWidget::NodeSelectedEvent,  (vtkCommand *)this->GUICallbackCommand );
    }
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::RemoveLogicObservers ( )
{
  //vtkSlicerApplicationGUI *appGUI = this->GetApplicationGUI();
  if (this->GetLogic())
    {
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::AddGUIObservers ( )
{
  this->RemoveGUIObservers();
  ...
  this->LITTPlanManagerSelectorWidget->AddObserver ( vtkSlicerNodeSelectorWidget::NodeSelectedEvent, (vtkCommand *)this->GUICallbackCommand );
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::AddLogicObservers ( )
{
  this->RemoveLogicObservers();  

  if (this->GetLogic())
    {
    this->GetLogic()->AddObserver(vtkLITTPlanLogic::StatusUpdateEvent,
                                  (vtkCommand *)this->LogicCallbackCommand);
    }
}

void vtkLITTPlanGUI::AddMRMLObservers(void)
{
...
void vtkLITTPlanGUI::RemoveMRMLObservers(void)
{
  if (this->MRMLScene!=NULL)
  {
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::HandleMouseEvent(vtkSlicerInteractorStyle *style)
{
}


//---------------------------------------------------------------------------
void vtkLITTPlanGUI::ProcessGUIEvents(vtkObject *caller,
                                         unsigned long event, void *callData)
{
  vtkMRMLLITTPlanManagerNode *manager=this->GetLITTPlanManagerNode();
  ...
  else if (this->LITTPlanManagerSelectorWidget == vtkSlicerNodeSelectorWidget::SafeDownCast(caller) &&
           (event == vtkSlicerNodeSelectorWidget::NodeSelectedEvent || event == vtkSlicerNodeSelectorWidget::NewNodeEvent )) 
    {
    vtkMRMLLITTPlanManagerNode *managerNode = vtkMRMLLITTPlanManagerNode::SafeDownCast(this->LITTPlanManagerSelectorWidget->GetSelected());
    char *managerID=NULL;
    if (managerNode!=NULL)
      {
      managerID=managerNode->GetID();
      }
    this->SetAndObserveLITTPlanManagerNodeID(managerID);    
    return;
    }
...
/*      vtkLITTPlanStep* currentLITTPlanStep=vtkLITTPlanStep::SafeDownCast(currentStep);
      if (currentLITTPlanStep)
      {
        //step->ShowUserInterface(); ShowUserInterface is triggered by state Enter event
        currentLITTPlanStep->UpdateGUI();
      }
*/
  ...
  // Process Wizard GUI (Active step only)
    ...
    if (manager)
      ...
      vtkLITTPlanStep *step=GetStepPage(stepId);
      if (step!=NULL)
        {
...
void vtkLITTPlanGUI::Init()
{
  ...
  // Register all new MRML node classes
  {
    ...
    this->GetMRMLScene()->RegisterNodeClass( vtkSmartPointer< vtkMRMLLITTPlanManagerNode >::New() );
    this->GetMRMLScene()->RegisterNodeClass( vtkSmartPointer< vtkMRMLRobotDisplayNode >::New() );
    this->GetMRMLScene()->RegisterNodeClass( vtkSmartPointer< vtkMRMLLaserApplicatorNode >::New() );
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::DataCallback(vtkObject *caller, 
        unsigned long eid, void *clientData, void *callData)
{
  vtkLITTPlanGUI *self = reinterpret_cast<vtkLITTPlanGUI *>(clientData);
  vtkDebugWithObjectMacro(self, "In vtkLITTPlanGUI DataCallback");
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::ProcessLogicEvents ( vtkObject *caller,
    unsigned long event, void *callData )
{

  if (this->GetLogic() == vtkLITTPlanLogic::SafeDownCast(caller))
    {
    if (event == vtkLITTPlanLogic::StatusUpdateEvent)
      {
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::ProcessMRMLEvents ( vtkObject *caller,
    unsigned long event, void *callData )
{

  // manager node chaged
  vtkMRMLLITTPlanManagerNode *manager=this->GetLITTPlanManagerNode();
  if (manager!=NULL && manager == vtkMRMLLITTPlanManagerNode::SafeDownCast(caller))
    {
    switch (event)
      {
  ...
  if ( this->MRMLScene!=NULL && vtkMRMLScene::SafeDownCast(caller) == this->MRMLScene && (event == vtkMRMLScene::NodeRemovedEvent ) )
    {
    if (this->LITTPlanManagerNodeID != NULL && this->MRMLScene->GetNodeByID(this->LITTPlanManagerNodeID) == NULL)
      {
...
void vtkLITTPlanGUI::Enter()
{
  ...
  if (this->MRMLScene!=NULL)
  {
    vtkMRMLLITTPlanManagerNode *anyManagerNode = vtkMRMLLITTPlanManagerNode::SafeDownCast(this->MRMLScene->GetNthNodeByClass(0, "vtkMRMLLITTPlanManagerNode"));
    if (anyManagerNode==NULL)
    {
      vtkSmartPointer<vtkMRMLLITTPlanManagerNode> newManagerNode=vtkSmartPointer<vtkMRMLLITTPlanManagerNode>::New();
  ...
  */
  ...
  if (this->WizardWidget!=NULL)
    ...
    if (wizard_workflow!=NULL)
      {
      vtkLITTPlanStep* step=vtkLITTPlanStep::SafeDownCast(wizard_workflow->GetCurrentStep());
      if (step)
        {
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::Enter(vtkMRMLNode *node)
{
  Enter();

  vtkMRMLLITTPlanManagerNode *managerNode = vtkMRMLLITTPlanManagerNode::SafeDownCast(node);
  if ( managerNode )
    {
    this->LITTPlanManagerSelectorWidget->UpdateMenu();
    this->LITTPlanManagerSelectorWidget->SetSelected( managerNode ); // :TODO: check if observers are updated after this
    }
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::BuildGUI ( )
{
    this->UIPanel->AddPage ( "LITTPlan", "LITTPlan", NULL );
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::TearDownGUI ( )
{
  ...
  if (this->WizardWidget!=NULL)
  {
    ...
    {
      vtkLITTPlanStep *step=vtkLITTPlanStep::SafeDownCast(this->WizardWidget->GetWizardWorkflow()->GetNthStep(i));
      if (step!=NULL)
        {
        step->TearDownGUI();
        step->SetGUI(NULL);
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::BuildGUIForWizardFrame()
{
    vtkKWWidget *page = this->UIPanel->GetPageWidget ( "LITTPlan" );
...
void vtkLITTPlanGUI::BuildGUIForHelpFrame ()
{
  ...
  helpss << "Module Version: " << vtkMRMLLITTPlanManagerNode::GetLITTPlanModuleVersion() << std::endl;
  helpss << "Source Revision: " << vtkMRMLLITTPlanManagerNode::GetLITTPlanWorkingCopyRevision() << std::endl;
  helpss << "The **LITTPlan Module** helps you to plan and navigate MRI-guided prostate biopsy ";
  helpss << "and brachytherapy using transrectal and transperineal needle placement devices. \n";
  helpss << "See <a>http://www.slicer.org/slicerWiki/index.php/Modules:LITTPlan-Documentation-3.6</a> for details.";
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::BuildGUIForConfigurationFrame ()
{
  vtkSlicerApplication *app = (vtkSlicerApplication *)this->GetApplication();
  vtkKWWidget *page = this->UIPanel->GetPageWidget ( "LITTPlan" );
  ...
  this->LITTPlanManagerSelectorWidget->GetWidget()->GetWidget()->SetWidth(24);
  this->LITTPlanManagerSelectorWidget->SetLabelText( "Exam: ");
  this->LITTPlanManagerSelectorWidget->NewNodeEnabledOn();
  this->LITTPlanManagerSelectorWidget->SetBalloonHelpString("Select the active LITTPlan configuration from the current scene.");
  this->Script("pack %s -side top -anchor nw -fill x -padx 2 -pady 2",
               this->LITTPlanManagerSelectorWidget->GetWidgetName());
  ...
  this->RobotSelectorWidget->AddNodeClass("vtkMRMLLaserApplicatorNode", NULL, NULL, "Laser Applicator");
...
//---------------------------------------------------------------------------
void vtkLITTPlanGUI::BuildGUIForWorkphaseFrame ()
{
...
//----------------------------------------------------------------------------
int vtkLITTPlanGUI::ChangeWorkphaseInGUI(int phase)
{
  vtkMRMLLITTPlanManagerNode *manager=this->GetLITTPlanManagerNode();
  if (manager==NULL)
  {
...
//----------------------------------------------------------------------------
void vtkLITTPlanGUI::UpdateGUI() // from MRML
{
  ...
  if (this->LITTPlanManagerSelectorWidget!= NULL)
  {
    vtkMRMLLITTPlanManagerNode *selManagerNode = vtkMRMLLITTPlanManagerNode::SafeDownCast(this->LITTPlanManagerSelectorWidget->GetSelected());
    if (selManagerNode!=this->GetLITTPlanManagerNode())
    {    
      this->LITTPlanManagerSelectorWidget->SetSelected(this->GetLITTPlanManagerNode());
    }
  }
...
//----------------------------------------------------------------------------
vtkLITTPlanStep* vtkLITTPlanGUI::GetStepPage(int i)
{
  if (this->WizardWidget==NULL)
    {
...
//----------------------------------------------------------------------------
void vtkLITTPlanGUI::UpdateStatusButtons()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanGUI::UpdateWorkflowSteps()
{
  bool changed=true;
  vtkMRMLLITTPlanManagerNode *manager=this->GetLITTPlanManagerNode();
  ...
  if (this->WizardWidget)
  {
    ...
    // Hide current step
    vtkLITTPlanStep* currentStep=vtkLITTPlanStep::SafeDownCast(wizard_workflow->GetCurrentStep());
    if (currentStep)
    {
  ...
  for (int i = 0; i < numSteps; i ++)
  {
    ...
    vtkLITTPlanStep* newStep=NULL;

    if (!stepName.compare("SetUp"))
    {
      vtkLITTPlanStepSetUp* setupStep = vtkLITTPlanStepSetUp::New();
      setupStep->SetTitleBackgroundColor(205.0/255.0, 200.0/255.0, 177.0/255.0);
      newStep=setupStep;
    } 
    else if (!stepName.compare("SetUpTemplate"))
      {
      vtkLITTPlanStepSetUpTemplate* setupStep = vtkLITTPlanStepSetUpTemplate::New();
      setupStep->SetTitleBackgroundColor(205.0/255.0, 200.0/255.0, 177.0/255.0);
      newStep=setupStep;
      }
    else if (!stepName.compare("ZFrameCalibration"))
    {
      vtkLITTPlanCalibrationStep* calibrationStep = vtkLITTPlanCalibrationStep::New();
    ...
    {
      vtkLITTPlanFiducialCalibrationStep* calibrationStep = vtkLITTPlanFiducialCalibrationStep::New();
      calibrationStep->SetTitleBackgroundColor(193.0/255.0, 115.0/255.0, 80.0/255.0);
      newStep=calibrationStep;
    }
    else if (!stepName.compare("PointTargetingWithoutOrientation"))
    {
      vtkLITTPlanTargetingStep* targetingStep = vtkLITTPlanTargetingStep::New();
    ...
    else if (!stepName.compare("PointTargeting"))
    {
      ...
      {
      vtkLITTPlanStepTargetingTemplate* targetingStep = vtkLITTPlanStepTargetingTemplate::New();
      targetingStep->SetTitleBackgroundColor(138.0/255.0, 165.0/255.0, 111.0/255.0);
      newStep=targetingStep;
      }
    else if (!stepName.compare("TransperinealProstateRobotManualControl"))
    {
      vtkLITTPlanManualControlStep* manualStep = vtkLITTPlanManualControlStep::New();
      manualStep->SetTitleBackgroundColor(179.0/255.0, 179.0/255.0, 230.0/255.0);
      newStep=manualStep;
    }
    else if (!stepName.compare("PointVerification"))
    {
      vtkLITTPlanStepVerification* verificationStep = vtkLITTPlanStepVerification::New();
    ...
    if (newStep!=NULL)
    {
      ...
      newStep->SetLITTPlanManager(manager);
...
//--------------------------------------------------------------------------------
void vtkLITTPlanGUI::BringTargetToViewIn2DViews(int mode)
{
  vtkMRMLLITTPlanManagerNode *manager=GetLITTPlanManagerNode();
  if(manager==NULL)
  {
    return;
  }
  ...
  vtkLITTPlanTargetDescriptor* targetDesc=manager->GetTargetDescriptorAtIndex(currentTargetInd);
...
//--------------------------------------------------------------------------------
void vtkLITTPlanGUI::BringMarkerToViewIn2DViews(double* P, double* N/*=NULL*/, double* T/*=NULL*/)
{
...
//--------------------------------------------------------------------------------
void vtkLITTPlanGUI::UpdateCurrentTargetDisplay()
{
  vtkMRMLLITTPlanManagerNode *manager=GetLITTPlanManagerNode();
  if(manager==NULL)
  {
...
void vtkLITTPlanGUI::UpdateCurrentTargetDisplayInSecondaryWindow()
{  
  vtkMRMLLITTPlanManagerNode *manager=GetLITTPlanManagerNode();
  if(manager==NULL)
  {
  ...
  vtkMRMLRobotNode* robot=manager->GetRobotNode();
  vtkLITTPlanTargetDescriptor *targetDesc = manager->GetTargetDescriptorAtIndex(manager->GetCurrentTargetIndex()); 
  NeedleDescriptorStruct *needle=manager->GetNeedle(targetDesc);
  std::string mainInfo;
...
//----------------------------------------------------------------------------
void vtkLITTPlanGUI::SetAndObserveRobotNodeID(const char *nodeID)
{
  bool modified=false;
...
//----------------------------------------------------------------------------
vtkMRMLRobotNode* vtkLITTPlanGUI::GetRobotNode()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanGUI::SetAndObserveLITTPlanManagerNodeID(const char *nodeID)
{
  bool modified=false;
  if (nodeID!=NULL && this->LITTPlanManagerNodeID!=NULL)
  {
    if (strcmp(nodeID, this->LITTPlanManagerNodeID)!=0)
    {
      modified=true;
    }
  }
  else if (nodeID!=this->LITTPlanManagerNodeID)
  {
    modified=true;
  }

  vtkSetAndObserveMRMLObjectMacro(this->LITTPlanManagerNode, NULL);
  this->SetLITTPlanManagerNodeID(nodeID);
  vtkMRMLLITTPlanManagerNode *tnode = this->GetLITTPlanManagerNode();

  vtkSmartPointer<vtkIntArray> events = vtkSmartPointer<vtkIntArray>::New();
  events->InsertNextValue(vtkCommand::ModifiedEvent);
  events->InsertNextValue(vtkMRMLLITTPlanManagerNode::CurrentTargetChangedEvent);
  vtkSetAndObserveMRMLObjectEventsMacro(this->LITTPlanManagerNode, tnode, events);
  ...
  if (this->LITTPlanManagerNode != NULL)
    {
    const char regSectionName[]="LITTPlan";
    const char regDefaultNeedleListKeyName[]="LITTPlan";      
    ...
    if (this->GetApplication()->HasRegistryValue (2, regSectionName, regDefaultNeedleListKeyName))
      ...
      this->LITTPlanManagerNode->Init(defNeedleDesc);
      delete[] defNeedleDesc;
      defNeedleDesc=NULL;
      }
    else
      {      
      this->GetApplication()->SetRegistryValue(2,"LITTPlan","DefaultNeedleList","%s",DEFAULT_NEEDLE_DESCRIPTION);
      this->LITTPlanManagerNode->Init(DEFAULT_NEEDLE_DESCRIPTION);          
      }
  ...
  if (this->LITTPlanManagerNode!=NULL)
  {
    SetAndObserveRobotNodeID(this->LITTPlanManagerNode->GetRobotNodeID());
    SetAndObserveTargetPlanListNodeID(this->LITTPlanManagerNode->GetTargetPlanListNodeID());
  }
  else
  {
  ...
  if (this->WizardWidget!=NULL)
  {
    for (int i=0; i<this->WizardWidget->GetWizardWorkflow()->GetNumberOfSteps(); i++)
    {
      ...
      if (step!=NULL)
        {
        step->SetLITTPlanManager(this->LITTPlanManagerNode);
        }
      else
        {
        vtkErrorMacro("Invalid step page: "<<i);
...
//----------------------------------------------------------------------------
vtkMRMLLITTPlanManagerNode* vtkLITTPlanGUI::GetLITTPlanManagerNode()
{
  vtkMRMLLITTPlanManagerNode* node = NULL;
  if (this->MRMLScene && this->LITTPlanManagerNodeID != NULL )
    {
    vtkMRMLNode* snode = this->MRMLScene->GetNodeByID(this->LITTPlanManagerNodeID);
    node = vtkMRMLLITTPlanManagerNode::SafeDownCast(snode);
    }
  return node;
}

//----------------------------------------------------------------------------
void vtkLITTPlanGUI::SetAndObserveTargetPlanListNodeID(const char *nodeID)
{
  bool modified=false;
...
//----------------------------------------------------------------------------
vtkMRMLFiducialListNode* vtkLITTPlanGUI::GetTargetPlanListNode()
{
...
//----------------------------------------------------------------------------
void vtkLITTPlanGUI::RequestRenderInViewerWidgets()
{
...
void vtkLITTPlanGUI::ShowSecondaryWindowCheckButtonCallback(int checked)
{
