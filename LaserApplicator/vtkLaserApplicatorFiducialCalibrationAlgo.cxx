...
// debug params
static const bool WRITE_DEBUG_IMAGES=true;
static const char DEBUG_IMAGE_PATH[]=".";
...
//----------------------------------------------------------------------------
vtkStandardNewMacro(vtkLaserApplicatorFiducialCalibrationAlgo);
vtkCxxRevisionMacro(vtkLaserApplicatorFiducialCalibrationAlgo, "$Revision: 1.0 $");
//------------------------------------------------------------------------------

vtkLaserApplicatorFiducialCalibrationAlgo::vtkLaserApplicatorFiducialCalibrationAlgo()
{
...
//----------------------------------------------------------------------------
void vtkLaserApplicatorFiducialCalibrationAlgo::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os,indent);
}

//--------------------------------------------------------------------------------------
bool vtkLaserApplicatorFiducialCalibrationAlgo::CalibrateFromImage(const TRLaserAblationCalibrationFromImageInput &input)
{ 
  ...
  this->SegmentAxis(input.MarkerInitialPositions[0], input.MarkerInitialPositions[1], input.VolumeIJKToRASMatrix, input.VolumeImageData,
    input.MarkerSegmentationThreshold[0], input.MarkerSegmentationThreshold[1], input.MarkerDimensionsMm, input.MarkerRadiusMm, input.RobotInitialAngle,
    P2, v2, this->MarkerPositions[0], this->MarkerPositions[1], this->MarkerFound[0], this->MarkerFound[1], CalibMarkerPreProcOutput[0], CalibMarkerPreProcOutput[1], &CoordinatesVectorAxis2);
...
//--------------------------------------------------------------------------------------
void vtkLaserApplicatorFiducialCalibrationAlgo::SegmentAxis(const double initPos1Ras[3], const double initPos2Ras[3], vtkMatrix4x4 *volumeIJKToRASMatrix, vtkImageData* calibVol,
...
bool vtkLaserApplicatorFiducialCalibrationAlgo::SegmentCircle(double markerCenterGuess_RAS[3],const double normal_RAS[3],  double thresh, const double fidDimsMm[3], 
...
bool vtkLaserApplicatorFiducialCalibrationAlgo::CalculateCircleCenter(vtkImageData *planeImageData, unsigned int *tempStorage, int tempStorageSize, double nThersholdVal, double nRadiusMm, double &gx, double &gy, int nVotedNeeded, bool lDebug)
...
bool vtkLaserApplicatorFiducialCalibrationAlgo::CalculateCircleCenterMean(vtkImageData *planeImageData, double nRadiusMm, double threshold, double &gx, double &gy)
{
    // spacing should not be 0!
    double spacing[3];
    planeImageData->GetSpacing(spacing);
...
bool vtkLaserApplicatorFiducialCalibrationAlgo::FindProbe(const double P1[3], const double P2[3], double v1[3], double v2[3], 
...
struct PointTooFar 
{
  ...
  bool operator()( itk::Point<double, 3> x) {
    return LITTPlanMath::PointDistanceFromLineM(x,defp_p,defv_p)>max_distance;
  }
};

/// Remove outlaying points
void vtkLaserApplicatorFiducialCalibrationAlgo::RemoveOutliners(double P_[3], double v_[3], const double def1[3], const double def2[3], std::vector<PointType> &CoordinatesVector)
{
    ...
    do {
        ...
        for (i=CoordVectorCopy.begin(); i!=CoordVectorCopy.end(); ++i){
            dist=LITTPlanMath::PointDistanceFromLineM(*i,P_,v_);
    ...
    // always use tip as the reference
    P_[0]=def1[0];
    P_[1]=def1[1];
    P_[2]=def1[2];
...
void vtkLaserApplicatorFiducialCalibrationAlgo::Linefinder(double P_[3], double v_[3], std::vector<itk::Point<double,3> > CoordVector)
{// Axel line algorithm
...
bool vtkLaserApplicatorFiducialCalibrationAlgo::FindTargetingParams(vtkLITTPlanTargetDescriptor *target, const TRLaserAblationCalibrationData &calibrationData, NeedleDescriptorStruct *needle, TRLaserAblationTargetingParams *targetingParams)
{
  if (target==NULL)
  {
    return false;
...
bool vtkLaserApplicatorFiducialCalibrationAlgo::RotatePoint(double H_before[3], double rotation_rad, double alpha_rad, double mainaxis[3], double I[3], /*out*/double H_after[3])
{
        double skew[3][3] = {
...
//----------------------------------------------------------------------------
bool vtkLaserApplicatorFiducialCalibrationAlgo::DoubleEqual(double val1, double val2)
{
  bool result = false;
...
//----------------------------------------------------------------------------
vtkImageData* vtkLaserApplicatorFiducialCalibrationAlgo::GetCalibMarkerPreProcOutput(int i)
{
  return this->CalibMarkerPreProcOutput[i];
}

//----------------------------------------------------------------------------
vtkMatrix4x4* vtkLaserApplicatorFiducialCalibrationAlgo::GetCalibMarkerPreProcOutputIJKToRAS()
{
  return this->CalibMarkerPreProcOutputIJKToRAS;
}

//----------------------------------------------------------------------------
void vtkLaserApplicatorFiducialCalibrationAlgo::GetAxisCenterpoints(vtkPoints *points, int i)
{
  if (points==NULL)
    {
    // TODO: log error
...
//----------------------------------------------------------------------------
void vtkLaserApplicatorFiducialCalibrationAlgo::SetEnableMarkerCenterpointAdjustment(bool enable)
{
  this->EnableMarkerCenterpointAdjustment=enable;
}

void vtkLaserApplicatorFiducialCalibrationAlgo::CropWithCylinder(vtkImageData* output, vtkImageData* input, const double linePoint_RAS[3],const double lineVector_RAS[3], vtkMatrix4x4 *ijkToRAS, double radiusMm)
{
...
//----------------------------------------------------------------------------
bool vtkLaserApplicatorFiducialCalibrationAlgo::GetMarkerFound(int i)
{
  return this->MarkerFound[i];
}

//----------------------------------------------------------------------------
double* vtkLaserApplicatorFiducialCalibrationAlgo::GetMarkerPositions(int i)
{
