...
vtkCxxRevisionMacro(vtkLITTPlanLogic, "$Revision: 1.9.12.1 $");
vtkStandardNewMacro(vtkLITTPlanLogic);

//---------------------------------------------------------------------------
vtkLITTPlanLogic::vtkLITTPlanLogic()
{
  // Timer Handling
  this->DataCallbackCommand = vtkCallbackCommand::New();
  this->DataCallbackCommand->SetClientData( reinterpret_cast<void *> (this) );
  this->DataCallbackCommand->SetCallback(vtkLITTPlanLogic::DataCallback);
...
//---------------------------------------------------------------------------
void vtkLITTPlanLogic::PrintSelf(ostream& os, vtkIndent indent)
{
    this->vtkObject::PrintSelf(os, indent);

    os << indent << "vtkLITTPlanLogic:             " << this->GetClassName() << "\n";

}

//---------------------------------------------------------------------------
void vtkLITTPlanLogic::DataCallback(vtkObject *caller, 
                                       unsigned long eid, void *clientData, void *callData)
{
    vtkLITTPlanLogic *self = reinterpret_cast<vtkLITTPlanLogic *>(clientData);
    vtkDebugWithObjectMacro(self, "In vtkLITTPlanLogic DataCallback");
    self->UpdateAll();
}

//---------------------------------------------------------------------------
void vtkLITTPlanLogic::UpdateAll()
{

}


int vtkLITTPlanLogic::Enter()
{      
...
//----------------------------------------------------------------------------
void vtkLITTPlanLogic::TimerHandler()
{
  if (this->TimerOn)
  {
...
//---------------------------------------------------------------------------
int vtkLITTPlanLogic::RobotStop()
{

  std::cerr << "vtkLITTPlanLogic::RobotStop()" << std::endl;
  return 1;
...
//---------------------------------------------------------------------------
int vtkLITTPlanLogic::RobotMoveTo(float px, float py, float pz,
                                     float nx, float ny, float nz,
                                     float tx, float ty, float tz)
{

  std::cerr << "vtkLITTPlanLogic::RobotMoveTo()" << std::endl;
  return 1;
}


//---------------------------------------------------------------------------
int vtkLITTPlanLogic::RobotMoveTo(float position[3], float orientation[3])
{
  std::cerr << "vtkLITTPlanLogic::RobotMoveTo()" << std::endl;
...
//---------------------------------------------------------------------------
int vtkLITTPlanLogic::RobotMoveTo()
{
  vtkMRMLRobotNode* robot=GetRobotNode();
  if (robot==NULL)
  {
...
//---------------------------------------------------------------------------
int vtkLITTPlanLogic::ScanStart()
{
...
//---------------------------------------------------------------------------
int vtkLITTPlanLogic::ScanPause()
{
  return 1;
}

//---------------------------------------------------------------------------
int vtkLITTPlanLogic::ScanStop()
{
...
//---------------------------------------------------------------------------
void vtkLITTPlanLogic::SetSliceViewFromVolume(vtkMRMLVolumeNode *volumeNode)
{
  if (!volumeNode)
    {
    return;
...
//             "   " << matrix->GetElement(0,1) <<
  ...
  int permutation[3];
  int flip[3];
  LITTPlanMath::ComputePermutationFromOrientation(matrix, permutation, flip);
...
//---------------------------------------------------------------------------
vtkMRMLScalarVolumeNode *vtkLITTPlanLogic::AddVolumeToScene(const char *fileName, VolumeType volumeType/*=VOL_GENERIC*/)
{
  if (fileName==0)
  {
  ...
  vtkMRMLLITTPlanManagerNode* manager=this->GUI->GetLITTPlanManagerNode();
  if (manager==NULL)
  {
...
//---------------------------------------------------------------------------
int vtkLITTPlanLogic::SelectVolumeInScene(vtkMRMLScalarVolumeNode* volumeNode, VolumeType volumeType)
{
  if (volumeNode==0)
  {
...
//---------------------------------------------------------------------------
vtkMRMLScalarVolumeNode *vtkLITTPlanLogic::AddArchetypeVolume(const char* fileName, const char *volumeName)
{
  // Set up storageNode
...
//--------------------------------------------------------------------------------------
std::string vtkLITTPlanLogic::GetFoRStrFromVolumeNodeID(const char* volNodeID)
{  
...
void vtkLITTPlanLogic::UpdateTargetListFromMRML()
{
  vtkMRMLLITTPlanManagerNode* manager=this->GUI->GetLITTPlanManagerNode();
  if (manager==NULL)
  {
  ...
  {
    vtkLITTPlanTargetDescriptor *t=manager->GetTargetDescriptorAtIndex(i);
  ...
  for (int i=0; i<fidNode->GetNumberOfFiducials(); i++)
  {
    ...
    if (targetIndex<0)
    {
      ...
      vtkSmartPointer<vtkLITTPlanTargetDescriptor> targetDesc=vtkSmartPointer<vtkLITTPlanTargetDescriptor>::New();      
    ...
    {
      // Update fiducial
      vtkLITTPlanTargetDescriptor* targetDesc = manager->GetTargetDescriptorAtIndex(targetIndex);
      if (targetDesc!=NULL)
      {
...
//----------------------------------------------------------------------------
void vtkLITTPlanLogic::LinkTargetsToFiducials()
{
  vtkMRMLLITTPlanManagerNode* manager=this->GUI->GetLITTPlanManagerNode();
  if (manager==NULL)
  {
  ...
  {
    vtkLITTPlanTargetDescriptor *t=manager->GetTargetDescriptorAtIndex(targetInd);
...
//----------------------------------------------------------------------------
int vtkLITTPlanLogic::GetTargetIndexFromFiducialID(const char* fiducialID)
{
  if (fiducialID==NULL)
  {
...
//----------------------------------------------------------------------------
int vtkLITTPlanLogic::SetMouseInteractionMode(int mode)
{  
...
//----------------------------------------------------------------------------
int vtkLITTPlanLogic::SetCurrentFiducialList(vtkMRMLFiducialListNode* fidNode)
{
  if (fidNode==NULL)
  {
...
vtkMRMLRobotNode* vtkLITTPlanLogic::GetRobotNode()
{
  if (this->GUI==NULL)
  {
    return NULL;
  }
  if (this->GUI->GetLITTPlanManagerNode()==NULL)
  {
    return NULL;
  }
  return this->GUI->GetLITTPlanManagerNode()->GetRobotNode();
  {
    return NULL;
  }
}
...
//----------------------------------------------------------------------------
int vtkLITTPlanLogic::ShowWorkspaceModel(bool show)
{
  vtkMRMLRobotNode* robot=GetRobotNode();
  if (robot==NULL)
  {
...
//----------------------------------------------------------------------------
bool vtkLITTPlanLogic::IsWorkspaceModelShown()
{
  vtkMRMLRobotNode* robot=GetRobotNode();
  if (robot==NULL)
  {
...
//----------------------------------------------------------------------------
int vtkLITTPlanLogic::ShowRobotModel(bool show)
{
  vtkMRMLRobotNode* robot=GetRobotNode();
  if (robot==NULL)
  {
...
//----------------------------------------------------------------------------
bool vtkLITTPlanLogic::IsRobotModelShown()
{
  vtkMRMLRobotNode* robot=GetRobotNode();
  if (robot==NULL)
  {
